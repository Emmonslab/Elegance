...
class Synapses
{
    ...
    while (rs.next()) 
     {
    	...
    	if (rs.getInt(2) != n1) 
	      {
    		if (rs.getInt(2) == 4685) System.out.println("4685  "+Integer.toString(n1)+"   "+post);
    		if (rs.getInt(2) == 4686) System.out.println("4686  "+Integer.toString(n1)+"   "+post);
    		if (rs.getInt(2) == 4687) System.out.println("4687  "+Integer.toString(n1)+"   "+post);
	...
	{
		System.out.println("begin to combine");
		...
		if (objPostInd.containsKey(syn.lastpre)) 
		{
				...
				
			{
				if(syn.lastpre.equals("4685")&& syn.synID.equals("11116")) 
					{
					flagg = 1;
					System.out.println(syn.synID+"   "+syn.lastpre+" relid:  "+relids[i]+"  "+relids.length+"   "+i);
					}
				Rel rel = (Rel) rels.get(relids[i]);
				String next = rel.getObjName2();
				//System.out.println("find a next"+next);
				if(pres.containsKey(next))
				{
					//System.out.println("find a next syn"+next);
					...
					for(int j=0;j<synids.length;j++)
					{
						...
						//System.out.println("find a next syn "+synnext.post);
						if((syn.type.equals("chemical"))&& (synnext.type.equals("chemical")))
						{
							
							if (flagg==1) System.out.println(syn.lastpostname+"  "+synnext.firstpostname );
							...
							//if (syn1==11120 && syn2==11111) System.out.println(syn.lastpre+"       "+relids[i]+"          "+next+"        "+synids[j]+"      "+syn.members+"      "+synnext.members+"  combined!!!" );
							...
							//System.out.println("find a combine"+synnext);
							else if (compareTwoSyn(syn, synnext)) 
							{
								
								int syn1 = Integer.parseInt(syn.lastmember);
								int syn2 = Integer.parseInt(synnext.firstmember);
								if (flagg==1) System.out.println(syn.synID+"    "+syn.lastpre+"       "+i+"       "+(String)objPostInd.get(syn.lastpre)+"      "+relids[i]+"          "+next+"        "+synids[j]+"      "+syn.members+"      "+synnext.members+"  combined!!!" );
						...
						else if ((syn.type.equals("electrical"))&&(synnext.type.equals("electrical")))
						{
							...
							if (((syn.postname.trim()).equals(synnext.postname.trim()))&&(syn.prename.trim().equals(synnext.prename.trim())))
							{
								...
								count1++;
								//System.out.println("syn post: "+syn.synID+" "+syn.postname+" "+syn.lastImage+"synnext post: "+synnext.synID+" "+synnext.postname);
								//System.out.println("syn pre: "+syn.synID+" "+syn.prename+" syn pre: "+synnext.synID+" "+synnext.prename);
							}
							//
							//System.out.println("syn pre: "+syn.prename+" synnext pre: "+synnext.prename);}
						}
	...
	public Synapses()
	{
		...
		try {
			...
			System.out.println("It took "+(time3-time2)/1000+" to calculate combining "+count1+" unconnected adjacent gap junctions");
			
			save();
			long time4 = System.currentTimeMillis();
			System.out.println("It took "+(time4-time3)/1000+" to save, done");
