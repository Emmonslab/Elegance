...
public TwoDPAG(int[] continNums, String dtype,int zoom,int stroke)throws SQLException,
		...
		{	  Class.forName("com.mysql.jdbc.Driver").newInstance();		
	          ...
	          for (int i = 0; i < continNums.length; i++) {
				...
				while (rs.next()) {
					...
					if (i==0) continName = continNames[0];
...
// find out the max and min X and Y
	...
	zoomX=height/3000.0000d;
	zoomY=height/4000.0000d;
  ...
  //show title
    ...
    int scaleLength = (maxSection -minSection)/50;
	for (int f=0;f<=scaleLength+1;f++ ){
	double start = 50-(minSection-Math.floor(minSection/50)*50)*zoomSection;
    l.setLine((double)(start+f*50*zoomSection),(double)(height*0.95),(double)(start+f*50*zoomSection),(double)(height*0.95-5));
	g2.setPaint(Color.black);
    g2.draw(l);

	l.setLine((double)(start+f*(50*zoomSection)),(double)(height*0.95),(double)(start+(f+1)*(50*zoomSection)),(double)(height*0.95));
	g2.setPaint(Color.black);
    g2.draw(l);
	...
	g2.drawString(scaleNum.toString(),(float)(start+f*(50*zoomSection)),(float)(height*0.95-10));
	...
	int branch1,branch2;
	String remark1,remark2,objName1,objName2;
	

	Color[] color = {Color.blue,Color.red,Color.green,Color.magenta,Color.orange,Color.black,Color.yellow,Color.cyan,Color.DARK_GRAY,Color.pink,Color.gray,Color.lightGray};
	...
	for (int i = 0; i < continNums.length; i++) {
		...
		g2.setFont(font2);
		g2.drawString(" in "+series[i],(float)(150*zoom),(float)(30+30*i*zoom));
		...
		jsql = "select x1,y1,z1,remarks1,cellbody1,branch1,objName1,x2,y2,z2,remarks2,cellbody2,branch2,objName2 from display2 where continNum="+ continNums[i];
		...
		while (rs.next()) {
			...
			x1 = (double)rs.getInt(1)*zoomX/1.2;
			...
			x2 = (double)rs.getInt(8)*zoomX/1.2;
