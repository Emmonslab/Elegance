...
class TwoDCurve {
	...
	public TwoDCurve(int[] continNums, String dtype, int zoom, int stroke) throws SQLException, ClassNotFoundException, java.lang.InstantiationException,
...

	{
		...
		setContinName();
		setDimension();
		if (dtype.equals("Z_Y") || dtype.equals("Z_X"))
			setSyns();
	...
	public void setSyns() throws SQLException, ClassNotFoundException, java.lang.InstantiationException, java.lang.IllegalAccessException {
		...
		try {
			...
			for (int i = 0; i < continNames.length; i++) {
				...
				rs = pst.executeQuery();
				while (rs.next()) {
					String type = rs.getString("type"), pre = rs.getString("pre"), post = rs.getString("post"), post1 = rs.getString("post1"), post2 = rs
							.getString("post2"), post3 = rs.getString("post3"), post4 = rs.getString("post4"),

					preobj = rs.getString("preobj"), postobj1 = rs.getString("postobj1"), postobj2 = rs.getString("postobj2"), postobj3 = rs
							.getString("postobj3"), postobj4 = rs.getString("postobj4");

					int sections = rs.getInt("sections"), partnerNum = rs.getInt("partnerNum");
					String name = "";
	...
	public void setContinName() {
		...
		try {
			...
			for (int i = 0; i < continNums.length; i++) {
				...
				rs = st.executeQuery("select CON_AlternateName,series from contin where CON_Number=" + continNums[i]);
				while (rs.next()) {
					continNames[i] = rs.getString(1);
					// Util.info(continNames[i]);
	...
	public void setDimension() {
		...
		width = (int) zoom * 1024;
		height = (int) zoom * 768;
		ELog.info("maxSection=" + maxSection);
		ELog.info("minSection=" + minSection);
		zoomSection = width / ((maxSection - minSection) * 1.2);

		zoomX = height / 3000.0000d; // the width of image , need to be change
										// here if different
		zoomY = height / 4000.0000d; // the height of image , need to be change
										// here if different
	...
	public BufferedImage getBufferedImage() {
		...
		for (int f = 0; f <= scaleLength + 1; f++) {
			...
			l.setLine((double) (start + f * 100 * zoomSection), (double) (height * 0.95), (double) (start + f * 100 * zoomSection),
					(double) (height * 0.95 - 5));
			g2.setPaint(Color.black);
			g2.draw(l);

			l.setLine((double) (start + f * (100 * zoomSection)), (double) (height * 0.95), (double) (start + (f + 1) * (100 * zoomSection)),
					(double) (height * 0.95));
			g2.setPaint(Color.black);
			g2.draw(l);
			if (f % 2 == 0) {
		...
		Color[] color = { Color.blue, Color.red, Color.green, Color.magenta, Color.orange, Color.black, Color.yellow, Color.cyan, Color.DARK_GRAY, Color.pink,
		...
		Color filterColor=Color.blue;
		
		try {
			con = EDatabase.borrowConnection();// Util.info(continNums[0]+continNums[1]);
			int cint = 0;
			for (int i = 0; i < continNums.length; i++) {
				...
				jsql = "select x1,y1,z1,remarks1,cellbody1,branch1,objName1,x2,y2,z2,remarks2,cellbody2,branch2,objName2,series1,series2 from display2 where continNum="
						+ continNums[i];
				pstmt = con.prepareStatement(jsql);
				rs = pstmt.executeQuery();
				while (rs.next()) {
					...
					if (!remark1.equals("") && !remark1.equals("null") && remark1 != null) {
						g2.setFont(font);
						g2.setPaint(color[i]);
						g2.drawString(remark1, (float) (((Point2D.Double) point1).x), (float) (((Point2D.Double) point2).y - 5));
					...
					for (int j = 0; j < serie.length; j++) {
						// Util.info("sereie= "+serie[j]);
						if (series1.equals(serie[j]))
							g2.setColor(scolor[j]);
					}
					g2.draw(l);
